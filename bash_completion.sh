...
complete -f -X '!*.@(zip|[ejw]ar|exe|o[tx]t|od[fgpst]|epub)' unzip
complete -f -X '!*.@(Z|[gGd]z|t[ag]z)' gunzip zcat
complete -f -X '*.@(o|so|so.!(conf)|a|[rs]pm|gif|jp?(e)g|mp3|mp?(e)g|avi|asf|ogg|class)' vi vim view

# start of section containing compspecs that can be handled within bash

# user commands see only users
complete -u su write w runuser
...
complete -F _known_hosts ping nslookup mtr
...
complete -F _command command do else eval exec nohup then time xargs
...
for i in awk bash bc cat cp curl cut date df diff du grep head less ln ls \
  md5sum mkdir mkfifo mv netstat patch rm sed seq sha{,1,256,384,512}sum sort \
  tail tee touch tr uname uniq wc wget who
do
...
list="unzip gunzip zcat vi vim view"
eval complete -r ${list[@]}
# install new compspecs
eval complete -F _filedir_xspec "${list[@]}"
unset list

for app in bash-builtins brew coreutils findutils git-completion.bash gzip lsof make man net-tools python rsync ssh tar youtube-dl.bash-completion
do
    if [ -f "$BASH_COMPLETION_DIR/$app" ]
    then
